 ...
 */
public abstract class AbstractDrugOrderSubmissionElement implements HtmlGeneratorElement, FormSubmissionControllerAction, GettingExistingOrder {
	
	protected final Log log = LogFactory.getLog(AbstractDrugOrderSubmissionElement.class);
	...
	protected ErrorWidget drugErrorWidget;
	...
	protected DropdownWidget frequencyWeekWidget;
	
	protected ErrorWidget frequencyWeekErrorWidget;
	
	protected TextFieldWidget instructionsWidget;
	
	protected ErrorWidget instructionsErrorWidget;
	
	protected String instructionsLabel;
	
	protected List<String> drugLabels;
	...
	protected Double defaultDose;
	
	protected DrugOrder existingOrder;
	
	protected List<Drug> drugsUsedAsKey;
	
	public AbstractDrugOrderSubmissionElement(FormEntryContext context, Map<String, String> parameters) {
		...
		if (usingDurationField) {
			...
			if (existingOrder != null && existingOrder.getAutoExpireDate() != null) {
				...
				Long startDateMilis = compat.getStartDate(existingOrder).getTime();
	...
	protected abstract void createAdditionalWidgets(FormEntryContext context);
	 ...
	 */
	protected abstract void populateDrugOrderValuesFromDB(FormEntryContext context, Boolean usingDurationField);
	...
	protected abstract void editOrder(FormEntrySession session, OrderTag orderTag);
	
	protected abstract void enterOrder(FormEntrySession session, OrderTag orderTag);
