...
/**
 * Helper for standardRegimen tag. NOTE: matching regimens to existing DrugOrders does NOT take dose
 * or frequency into account. All matching is by drug, start date, discontinued date.
 * 
 * @author dthomas
 */
public class StandardRegimenUtil {
	
	/**
	 * Finds 'strongest' RegimenSuggestion match, and returns map with size 0 or 1. If 1, map contains
	 * the RegimenSuggestion and the list of DrugOrders. 'strongest' = if you have standard regimen A
	 * containing 1 drug, and you have standard regimen B containing the same drug plus one other drug,
	 * then if both standard regimens pass the 'match' test, this method will return standard regimen B
	 * because of the greater number of matches. In the event of a tie, the first match with the
	 * greatest 'strength' gets returned. NOTE: supports drugId in standard regimen XML being a uuid
	 * (matches on drug.uuid).
	 * 
	 * @param regimenCandidates
	 * @param drugOrders
	 * @return Map<RegimenSuggestion, List<DrugOrder>>
	 */
	public static Map<RegimenSuggestion, List<DrugOrder>> findStrongestStandardRegimenInDrugOrders(
	        List<RegimenSuggestion> regimenCandidates, List<Order> drugOrders) {
				...
				for (DrugSuggestion dc : rs.getDrugComponents()) {
					...
					if (dorToInspect != null) { // there was a match by drug
						if (startDate == null) // setup standard regimen start date
							startDate = compat.getStartDate(dorToInspect);
						if (startDate.equals(compat.getStartDate(dorToInspect))) //this will always be true for first drug matched.
	...
	private static DrugOrder drugIdFoundInDrugSet(DrugSuggestion ds, List<Order> dors) {
		for (Order or : dors) {
			if (or instanceof DrugOrder) {
				...
				if (dor.getDrug() != null && !dor.isVoided() && (dor.getDrug().getDrugId().toString().equals(ds.getDrugId())
			...
			for (DrugSuggestion ds : rs.getDrugComponents()) {
				...
				if (drug == null)
					throw new RuntimeException(
					        "Your standard regimen xml file constains a drugId that can't be found, for regimen "
					                + rs.getCodeName() + ", DrugComponent.id = " + ds.getDrugId());
				dor.setDrug(Context.getConceptService().getDrugByNameOrId(ds.getDrugId()));
				compat.setFrequency(dor, ds.getFrequency());
				compat.setDoseUnits(dor, ds.getUnits());
				dor.setInstructions(ds.getInstructions());
				dor.setDose(Double.valueOf(ds.getDose()));
				compat.setStartDate(dor, startDate);
