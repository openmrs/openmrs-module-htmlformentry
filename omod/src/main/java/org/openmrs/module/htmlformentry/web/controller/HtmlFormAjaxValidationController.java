...
public class HtmlFormAjaxValidationController {
	...
	public void duplicateForm(ModelMap model, HttpServletRequest request, HttpServletResponse response,
	        ...
	        @RequestParam(required = false, value = "dateFormat") String dateFormat) throws Exception {
		...
		for (Encounter enc : encounters) {
			if (enc.getForm() != null) {
				...
				if (htmlForm != null && htmlForm.getId().equals(formId)
				        && dateToCheck.compareTo(enc.getEncounterDatetime()) == 0) {
					duplicate = true;
				}
			}
		}
