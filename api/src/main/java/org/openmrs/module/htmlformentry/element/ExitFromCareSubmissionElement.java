...
/**
 * Holds the widgets used to record details of exiting a person from care center, and serves as both
 * the HtmlGeneratorElement and the FormSubmissionControllerAction for exit from care functionality
 */
public class ExitFromCareSubmissionElement implements HtmlGeneratorElement, FormSubmissionControllerAction {
	...
	private void createElement(FormEntryContext context, Map<String, String> parameters) {
		...
		reasonForExitWidget.addOption(new Option(
		        Context.getMessageSourceService().getMessage("htmlformentry.chooseAReasonForExit"), "", !initialValueIsSet));
		...
		if (reasonExitConcept != null) {
			for (ConceptAnswer conceptAnswer : reasonExitConcept.getAnswers()) {
				...
				Option answerOption = new Option(answerConcept.getDisplayString(), answerConcept.getId().toString(),
				        answerConcept.equals(initialAnswer));
		...
		causeOfDeathWidget.addOption(new Option(
		        Context.getMessageSourceService().getMessage("htmlformentry.chooseACauseToDeath"), "", !causeOfDeathIsSet));
		
		if (causeOfDeathConcept != null) {
			for (ConceptAnswer conceptAnswer : causeOfDeathConcept.getAnswers()) {
				Concept answerConcept = conceptAnswer.getAnswerConcept();
				Option answerOption = new Option(answerConcept.getDisplayString(), answerConcept.getId().toString(),
				        answerConcept.equals(initialCauseOfDeath));
	...
	public Collection<FormSubmissionError> validateSubmission(FormEntryContext context, HttpServletRequest submission) {
		...
		if (dateWidget != null) {
			...
			if (obsList != null && obsList.size() == 0) {
				if (exitDate == null) {
					if (reasonForExitWidget != null && HtmlFormEntryUtil.convertToType(
					    reasonForExitWidget.getValue(context, submission).toString().trim(), Concept.class) != null) {
						ret.add(new FormSubmissionError(context.getFieldName(dateErrorWidget),
			...
			if (obsList != null && obsList.size() == 0) {
				if (exitReasonAnswerConcept == null) {
					if (dateWidget != null && dateWidget.getValue(context, submission) != null) {
						ret.add(new FormSubmissionError(context.getFieldName(reasonForExitErrorWidget),
						        Context.getMessageSourceService().getMessage("htmlformentry.error.required")));
					}
				} else if (exitReasonAnswerConcept != null
				        && exitReasonAnswerConcept.getConceptId().equals(patientDiedConcept.getConceptId())) {
					if (causeOfDeathWidget == null || HtmlFormEntryUtil.convertToType(
					    causeOfDeathWidget.getValue(context, submission).toString().trim(), Concept.class) == null) {
						ret.add(new FormSubmissionError(context.getFieldName(causeOfDeathErrorWidget),
						        Context.getMessageSourceService().getMessage("htmlformentry.error.required")));
					}
				} else if (exitReasonAnswerConcept != null
				        && !exitReasonAnswerConcept.getConceptId().equals(patientDiedConcept.getConceptId())) {
					if (causeOfDeathWidget != null && HtmlFormEntryUtil.convertToType(
					    causeOfDeathWidget.getValue(context, submission).toString().trim(), Concept.class) != null) {
						ret.add(new FormSubmissionError(context.getFieldName(causeOfDeathErrorWidget),
						        Context.getMessageSourceService().getMessage("htmlformentry.error.cannotEnterAValue")));
					}
				}
			}
		}
		...
		if (causeOfDeathWidget != null) {
			...
			if (obsDeath != null && obsDeath.size() == 0) {
				...
				} else if (causeOfDeathAnswerConcept != null
				        && causeOfDeathAnswerConcept.getConceptId().equals(otherNonCodedConcept.getConceptId())) {
					if (otherReasonWidget == null || valueString.equals("")) {
						ret.add(new FormSubmissionError(context.getFieldName(otherReasonErrorWidget),
						        Context.getMessageSourceService().getMessage("htmlformentry.error.required")));
					}
	...
	public String generateHtml(FormEntryContext context) {
		...
		if (reasonForExitWidget != null) {
			...
			if (context.getMode() == FormEntryContext.Mode.EDIT) {
				...
				sb.append("if (rVal == \"" + patientDiedConId + "\")\n" + " { jq(\"#" + causeWidgetId + "\").show();}\n"
				        ...
				        + "\").val(\"\"); jq(\"#" + causeWidgetId + "\").hide();" + " jq(\"#" + otherTextWidgetId
				        + "\").val(\"\"); jq(\"#" + otherTextWidgetId + "\").hide();}");
		...
		sb.append("if (reasonVal == \"" + patientDiedConId + "\")\n" + " { jq(\"#" + causeWidgetId + "\").show();}\n"
		        + "if (reasonVal != \"" + patientDiedConId + "\")\n" + " { jq(\"#" + causeWidgetId + "\").val(\"\"); jq(\"#"
		        + causeWidgetId + "\").hide();" + " jq(\"#" + otherTextWidgetId + "\").val(\"\"); jq(\"#" + otherTextWidgetId
		        + "\").hide();}");
		sb.append("});");
		sb.append("</script>");
		...
		if (causeOfDeathWidget != null) {
			...
			if (context.getMode() == FormEntryContext.Mode.ENTER) {
				sb.append("<script>");
				sb.append("jq(document).ready(function(){");
				sb.append(" if(jq(\"#" + causeWidgetId + "\").val() != \"\" && " + "jq(\"#" + reasonWidgetId
				        + "\").val() == \"" + patientDiedConId + "\"){\n");
			...
			if (context.getMode() == FormEntryContext.Mode.EDIT) {
				...
				sb.append("if (cVal == \"" + otherNonCodedConId + "\")\n" + " { jq(\"#" + otherTextWidgetId
				        + "\").show();}\n" + "if (cVal != \"" + otherNonCodedConId + "\")\n" + " { jq(\"#"
				        + otherTextWidgetId + "\").val(\"\"); jq(\"#" + otherTextWidgetId + "\").hide();}");
				sb.append("});");
				sb.append("</script>");
			}
		}
		...
		if (otherReasonWidget != null) {
			...
			if (context.getMode() == FormEntryContext.Mode.ENTER) {
				sb.append("<script>");
				sb.append(" if(jq(\"#" + otherTextWidgetId + "\").val() != \"\" &&" + " jq(\"#" + reasonWidgetId
				        + "\").val() == \"" + patientDiedConId + "\"){\n");
