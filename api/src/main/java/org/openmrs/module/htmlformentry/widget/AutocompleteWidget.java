...
public class AutocompleteWidget extends SingleOptionWidget {
	...
	/**
	 * @param context
	 * @return generated html as a string <strong>Should</strong> accept options with special characters
	 *         é,ã,ê,ù etc. <strong>Should</strong> accept options with single or double quotes in
	 *         middle <strong>Should</strong> correctly set previous value if initial option is present
	 */
	@Override
	public String generateHtml(FormEntryContext context) {
		...
		if (context.getMode() == FormEntryContext.Mode.VIEW) {
			...
			sb.append("\n<input type=\"hidden\" class=\"optionAutoCompleteHidden\" id=\"" + id + "_hid" + "\" name=\"" + id
			        + "\" value=\"" + ((initialOption != null) ? initialOption.getValue() : "") + "\" />");
			return sb.toString();
		}
	}
