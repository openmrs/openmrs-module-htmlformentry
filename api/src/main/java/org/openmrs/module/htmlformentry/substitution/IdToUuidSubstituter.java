...
/**
 * Performs Id to Uuid substitutions. Tests if the passed value is an id reference to an
 * OpenmrsObject. If so, returns the uuid of that object; otherwise, just returns the passed value.
 */
public class IdToUuidSubstituter implements Substituter {
	...
	public String substitute(String value, Class<?> clazz, Map<OpenmrsObject, OpenmrsObject> substitutionMap) {
		...
		if (value.matches("^\\d+$") && !HtmlFormEntryUtil.isValidUuidFormat(value)) {
			object = Context.getService(HtmlFormEntryService.class).getItemById((Class<? extends OpenmrsObject>) clazz,
