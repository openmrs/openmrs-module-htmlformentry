 ...
 * <p/>
 * It allows you to register widgets, which assigns them an id/name in the generated html, and
 * allows you to look up those id/names later. It allows you specify which error widget goes to
 * which widget, so that error messages get displayed in the right place.
 * <p/>
 * It also holds existing data for an encounter in View mode, so that widgets can be set with their
 * appropriate values.
 * <p/>
 ...
 * <p>
 * 'automaticClientSideValidation' means that elements and widgets should generate HTML that does
 * things like numeric range checking when you blur a text field. 'clientSideValidationHints' means
 * that elements and widgets should generate HTML where inputs have classes like "required" and
 * "numeric-range", and attributes like "min" and "max".
 * </p>
 */
public class FormEntryContext {
	 ...
	 *
	 * @param concept the concept associated with the Obs we are looking for
	 * @param answerDrug the Drug associated with the coded value of the Obs we are looking for (may be
	 *            null)
	 * @return the Obs from the current {@see ObsGroup} with the specified concept and answer concept
	 */
	 ...
	 * @param fieldFunctionName the name of the javascript function used to access the field itself (or
	 *            null to use the default function
	 * @param getterFunctionName the name of the javascript function used to get the value of the field
	 *            (or null to use the default function
	 * @param setterFunctionName the name of the javascript function used to set the value of the field
	 *            (or null to use the default function
	 */
	public void registerPropertyAccessorInfo(String property, String widgetId, String fieldFunctionName,
	        String getterFunctionName, String setterFunctionName) {
		if (widgetId == null)
			return;
