	...
	public String generateHtml(FormEntryContext context) {
		...
		if (context.getExistingPatient() != null && context.getExistingPatient().getId() != null) {
			...
			List<Relationship> existingRelationships = Context.getPersonService()
			        .getRelationshipsByPerson(context.getExistingPatient());
			for (int i = 0; i < relationshipsToCreate.size(); i++) {
						...
						if (side.equals("A")) {
							if (r.getPersonA().equals(context.getExistingPatient())) {
								...
								if (context.getMode() == Mode.VIEW) {
									sb.append(" ");
									sb.append(Context.getMessageSourceService()
									        .getMessage("htmlformentry.existingRelationshipsAdded"));
									sb.append(" - ");
				...
				context.registerPropertyAccessorInfo(parentId + "." + HtmlFormEntryUtil.toCamelCase(rl) + ".value",
				    context.getFieldNameIfRegistered(this), null, null, null);
				sb.append("</td>");
				sb.append("</tr>");
				if (!addComma) {
