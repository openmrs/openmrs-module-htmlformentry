 ...
 * <p/>
 * When going through XML/HTML substitution to build a form, one of these is created as a
 * side-effect.
 */
public class FormSubmissionController {
	...
	/**
	 * Marks the end of the a repeat. This has to be specified because nested repeating elements are not
	 * yet implemented.
	 */
	 ...
	 * <p/>
	 * This method cycles through all the FormSubmissionControllerActions and calls their
	 * validateSubmission method, adding any errors to the error list.
	 * 
	 ...
	 * <p/>
	 * This method cycles through all the FormSubmissionControllerActions and calls all their
	 * handleSubmission methods.
	 * 
			...
			try {
				...
				if (patient != null && encounter != null) {
					formObject = new SerializableFormObject(session.getXmlDefinition(), submission.getParameterMap(),
					        patient.getPatientIdentifier().getIdentifier(), patient.getUuid(), encounter.getUuid(),
					        session.getHtmlFormId());
				} else if (patient == null || encounter == null) {
					if (log.isDebugEnabled())
						log.debug("Either patient or encounter or both are null");
	...
	/**
	 * Serializes an object formed by pairing the HttpServletRequest & FormEntrySession objects
