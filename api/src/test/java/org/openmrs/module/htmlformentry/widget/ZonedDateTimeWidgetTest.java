...
import static org.openmrs.module.htmlformentry.HtmlFormEntryConstants.GP_FORMATTER_DATETIME;
...
public class ZonedDateTimeWidgetTest extends BaseHtmlFormEntryTest {
	...
	private ZonedDateTimeWidget widget;
	...
	public void before() throws ParseException {
		...
		administrationService.setGlobalProperty(GP_TIMEZONE_CONVERSIONS, "true");
		administrationService.setGlobalProperty(GP_FORMATTER_DATETIME, "dd-MM-yyyy, HH:mm:ss");
		administrationService.setGlobalProperty(DATETIME_FALLBACK_FORMAT, "dd-MM-yyyy, HH:mm:ss");
		widget = new ZonedDateTimeWidget();
	...
	@Test
	public void getValue_shouldReturnNullWhenDateIsClearedEvenIfTimeIsSet() {
		// setup
		HttpServletRequest request = mock(HttpServletRequest.class);
		when(request.getParameter("w1")).thenReturn("");
		when(request.getParameter("w1hours")).thenReturn("14");
		when(request.getParameter("w1minutes")).thenReturn("30");
		when(request.getParameter("w1seconds")).thenReturn("0");

		// replay
		Date result = widget.getValue(formEntryContext, request);
