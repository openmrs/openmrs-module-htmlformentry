...
public class HtmlFormEntryExportUtilTest extends BaseHtmlFormEntryTest {
	...
	public void getSectionAsForm_shouldReturnStringCorrectly() throws Exception {
		...
		TestUtil.assertFuzzyContains(
		    "ISONIAZID <select id=\"w2\" name=\"w2\" size=\"1\"><option value=\"\" selected=\"true\"></option><option value=\"2474\">Susceptible</option><option value=\"3017\">Intermediate</option><option value=\"1441\">Resistant</option></select>",
		    html);
	}
	...
	public void generateColumnHeadersFromHtmlForm_shouldReturnSchemaCorrectly() throws Exception {
		...
		String ret = HtmlFormEntryExportUtil.generateColumnHeadersFromHtmlForm(htmlform, extraCols, new StringBuffer(""),
		    Collections.singletonList(pit));
	...
	public void generateColumnDataFromHtmlForm_shouldReturnRowsCorrectly() throws Exception {
		...
		extraCols.add("comment");
		PatientIdentifierType pit = Context.getPatientService().getPatientIdentifierType(2);
		String header = HtmlFormEntryExportUtil.generateColumnHeadersFromHtmlForm(htmlform, extraCols, new StringBuffer(""),
		    Collections.singletonList(pit));
		...
		String ret = HtmlFormEntryExportUtil.generateColumnDataFromHtmlForm(encounters, htmlform, extraCols,
		    new StringBuffer(""), new Locale("en"), Collections.singletonList(pit));
	...
	public void generateColumnDataFromHtmlForm_shouldReturnRowsCorrectlyWithMultipleIDTypes() throws Exception {
		...
		StringBuffer ret = HtmlFormEntryExportUtil.buildHtmlFormExport(encounters, htmlform, extraCols, new StringBuffer(""),
	...
	public void getObsValueAsString_shouldRenderCorrectly() throws Exception {
		...
		Assert.assertTrue(ret.toString()
		        .contains("\"2003-02-01 00:00:00\",\"01/02/2003\",\"\",\"\",\"\",\"\",\"01/02/2003\",\"01/02/2003\","));
	...
	public void getSectionNodes_shouldReturnDropdownForConceptSelects() throws Exception {
		...
		Assert.assertTrue(xml.contains(
		    "<option value=\"\" selected=\"true\"></option><option value=\"2474\">Susc</option><option value=\"3017\">Interm</option>"));
		Assert.assertTrue(xml.contains(
		    "<option value=\"\" selected=\"true\"></option><option value=\"2474\">Susceptible</option><option value=\"3017\">Intermediate</option><option value=\"1441\">Resistant</option>"));
	}
