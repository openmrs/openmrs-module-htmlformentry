...
public class WidgetTest {
	...
	public void checkboxWidgetShouldProduceHtml() {
		...
		Assert.assertEquals(
		    "<input type=\"checkbox\" id=\"w1\" name=\"w1\" value=\"true\"/><input type=\"hidden\" name=\"_w1\"/>",
		    cw.generateHtml(context));
		cw.setInitialValue("Something");
		Assert.assertEquals(
		    "<input type=\"checkbox\" id=\"w1\" name=\"w1\" value=\"true\" checked=\"true\"/><input type=\"hidden\" name=\"_w1\"/>",
		    cw.generateHtml(context));
		cw.setLabel("This is a label");
		Assert.assertEquals(
		    "<input type=\"checkbox\" id=\"w1\" name=\"w1\" value=\"true\" checked=\"true\"/><label for=\"w1\">This is a label</label><input type=\"hidden\" name=\"_w1\"/>",
		    cw.generateHtml(context));
	}
	...
	public void dropdownWidgetShouldProduceHtml() {
		...
		Assert.assertEquals(
		    "<select id=\"w1\" name=\"w1\"><option value=\"XL\">Extra Large</option><option value=\"L\" selected=\"true\">Large</option></select>",
		    dw.generateHtml(context));
	}
	...
	public void checkboxWidgetShouldProduceHtmlWithToggleSimple() {
		...
		context.registerWidget(cw);
		Assert.assertEquals(
		    "<input type=\"checkbox\" id=\"w1\" name=\"w1\" value=\"true\" toggleHide=\"hatColors\"/><label for=\"w1\">Has a hat?</label><input type=\"hidden\" name=\"_w1\"/>",
		    cw.generateHtml(context));
	}
	...
	public void checkboxWidgetShouldProduceHtmlWithToggleDim() {
		...
		context.registerWidget(cw);
		Assert.assertEquals(
		    "<input type=\"checkbox\" id=\"w1\" name=\"w1\" value=\"true\" toggleDim=\"hatColors\"/><label for=\"w1\">Has a hat?</label><input type=\"hidden\" name=\"_w1\"/>",
		    cw.generateHtml(context));
	}
