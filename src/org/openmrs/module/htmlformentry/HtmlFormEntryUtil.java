...
public class HtmlFormEntryUtil {
	...
	/**
	 * Fetches a http request parameter from an http request and returns it as a
	 * specific type
	 * 
	 * @param request
	 ...
	 */
	public static Object getParameterAsType(HttpServletRequest request,
	 ...
	 * 
	 * @param formField
	 *            the form field that specifies the concept associated with the
	 *            Obs
	 * @param value
	 *            value associated with the Obs
	 * @param datetime
	 *            date/time associated with the Obs (may be null)
	 * @param accessionNumber
	 *            accession number associatd with the Obs (may be null)
	 * @return the created Obs instance
	 */

	public static Obs createObs(FormField formField, Object value,
			Date datetime, String accessionNumber) {
		Concept concept = formField.getField().getConcept();
		if (concept == null)
			throw new FormEntryException(
					"Can't create an Obs for a formField that doesn't represent a Concept");
	 ...
	 * 
	 * @param concept
	 *            concept associated with the Obs
	 * @param value
	 *            value associated with the Obs
	 * @param datetime
	 *            date/time associated with the Obs (may be null)
	...
	public static Obs createObs(Concept concept, Object value, Date datetime, String accessionNumber) {
        ...
        }//else the obs'value to remain  be null
        if (datetime != null)
	...
	public static String documentToString(Document document) throws Exception {
		...
		trans.setOutputProperty(OutputKeys.OMIT_XML_DECLARATION,
				HtmlFormEntryConstants.CONSTANT_YES);
		trans.setOutputProperty(OutputKeys.INDENT,
				HtmlFormEntryConstants.CONSTANT_YES);
		trans.setOutputProperty(OutputKeys.METHOD,
				HtmlFormEntryConstants.CONSTANT_XML);
	 ...
	 * 
	 * @param node
	 *            the Node to retrieve attributes from
	 ...
	 * 
	 * @param node
	 *            the Node to retrieve the attribute from
	 * @param attributeName
	 *            the name of the attribute to return
	 * @param defaultVal
	 *            a default value to return if the attribute is not specified
	...
	/**
	 * Creates a non-persistent "Fake" Person (used when previewing or
	...
	public static Patient getFakePerson() {
		...
		Location l = Context.getLocationService().getAllLocations().iterator()
				.next();
		for (PatientIdentifierType pit : Context.getPatientService()
			...
			if (StringUtils.isEmpty(pit.getValidator())) {
				demo.addIdentifier(new PatientIdentifier("Testing"
						+ pit.getName() + "123", pit, l));
			}
		}
		demo.setGender("f");
		{
		...
		for (PersonAttributeType type : Context.getPersonService()
				.getAllPersonAttributeTypes()) {
			if (type.getFormat().equals("java.lang.String")) {
				demo.addAttribute(new PersonAttribute(type, "Test "
	...
	/**
	 * Combines a Date object that contains only a date component (day, month,
	 * year) with a Date object that contains only a time component (hour,
	 * minute, second) into a single Date object
	 * 
	 * @param date
	 *            the Date object that contains date information
	 * @param time
	 *            the Date object that contains time information
