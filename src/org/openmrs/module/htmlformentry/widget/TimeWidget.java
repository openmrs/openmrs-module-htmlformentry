...
public class TimeWidget implements Widget {
	...
	public String generateHtml(FormEntryContext context) {
        if (context.getMode() == Mode.VIEW) {
            ...
            sb.append("<select name=\"").append(context.getFieldName(this)).append("hours").append("\">");
            for (int i = 0; i <= 23; ++i) {
            	String label = "" + i;
            	if (label.length() == 1)
            		label = "0" + label;
            ...
            sb.append("</select>");
            sb.append(":");
            sb.append("<select name=\"").append(context.getFieldName(this)).append("minutes").append("\">");
            for (int i = 0; i <= 59; ++i) {
            	String label = "" + i;
            	if (label.length() == 1)
            		label = "0" + label;
