...
public class VelocityFunctions {
	...
	private void cannotBePreviewed() {
		if ("testing-html-form-entry".equals(session.getPatient().getUuid()))
			throw new RuntimeException("Cannot be previewed"); 
	}
	...
	public Result logic(String expression) {
		...
		cannotBePreviewed();
		LogicCriteria lc = getLogicService().parseString(expression);
		try {
			return getLogicService().eval(session.getPatient(), lc);
		} catch (LogicException e) {
			// not sure if this is valid OpenMRS style
	...
	public PatientState currentProgramWorkflowStatus(Integer programWorkflowId) {
		...
		List<PatientProgram> pps = getProgramWorkflowService().getPatientPrograms(p, workflow.getProgram(), null, null, null, null, false);
		...
		for (PatientProgram pp : pps) {
			...
			if (!pp.isVoided()) {
				if (mostRecentPatientProgram == null) {
					...
					if (mostRecentPatientProgram.getDateCompleted() != null
							&& pp.getDateCompleted() == null) {
						// found an uncompleted one
						mostRecentPatientProgram = pp;
					} else if (mostRecentPatientProgram.getDateCompleted() != null
							&& pp.getDateCompleted() != null
