...
/**
 * Provides methods to take a {@code <htmlform>...</htmlform>} xml block and
 * turns it into HTML to be displayed as a form in a web browser. It can apply
 * the {@code <macros>...</macros>} section, and replace tags like {@code
 * <obs/>}.
 * 
 */
public class HtmlFormEntryGenerator implements TagHandler {
	...
	/**
	 * Takes an XML string, finds the {@code <macros></macros>} section in it,
	 * and applies those substitutions
	 * <p>
	 ...
	 * 
	 * @param xml
	 *            the xml string to process for macros
	...
	public String applyMacros(String xml) throws Exception {
		...
		// if there are no macros defined, we just return the original xml
		// unchanged
		if (macrosNode == null)
			return xml;
		...
		// switch back to String mode from the document so we can use string
		// utilities to substitute
	...
	/**
	 * Takes an XML string, finds the {@code <translations></translations>}
	 * section in it, and applies those substitutions
	 * 
	 * <pre>
	 * {@code
	 * <htmlform>
	 ...
	 * 
	 * @param xml
	 *            the xml string to process for translations
	 * @return the xml string after translation substitutions have been made
	 * @throws Exception
	 */
	public String applyTranslations(String xml, FormEntryContext context)
			throws Exception {
		Document doc = HtmlFormEntryUtil.stringToDocument(xml);
		Node content = HtmlFormEntryUtil.findChild(doc, "htmlform");
		Node transNode = HtmlFormEntryUtil.findChild(content, "translations");

		// if there are no translations defined, we just return the original xml
		// unchanged
		if (transNode == null) {
			return xml;
		}

		String defaultLocaleStr = HtmlFormEntryUtil.getNodeAttribute(transNode,
				"defaultLocale", "en");
		...
		for (int i = 0; i < codeNodeList.getLength(); i++) {
			...
			if (codeNode.getNodeName().equalsIgnoreCase("code")) {
				String codeName = HtmlFormEntryUtil.getNodeAttribute(codeNode,
						"name", null);
				if (codeName == null) {
					throw new IllegalArgumentException(
							"All translation elements must contain a valid code name");
				}
				...
				for (int j = 0; j < variantNodeList.getLength(); ++j) {
					...
					if (variantNode.getNodeName().equalsIgnoreCase("variant")) {
						String localeStr = HtmlFormEntryUtil.getNodeAttribute(
								variantNode, "locale", defaultLocaleStr);
						String valueStr = HtmlFormEntryUtil.getNodeAttribute(
								variantNode, "value", null);
						if (valueStr == null) {
							throw new IllegalArgumentException(
									"All variants must specify a value");
						}
						context.getTranslator().addTranslation(localeStr,
								codeName, valueStr);
					}
				}
			}
	...
	/**
	 * Takes an XML string, finds each {@code <repeat></repeat>} section in it,
	 * and applies those substitutions
	 * <p>
	 * 
	 * <pre>
	 ...
	 * 
	 * @param xml
	 *            the xml string to process for repeat sections
	...
	public String applyTemplates(String xml) throws Exception {
		...
		// We are doing this as follows since I can't seem to get the XML node
		// cloning to work right.
		// We can refactor later as needed if we can get it to work properly, or
		// replace the xml library
		// First we need to parse the document to get the node attributes for
		// repeating elements
		...
		// Now we are just going to use String replacements to explode the
		// repeat tags properly
		Iterator<List<Map<String, String>>> renderMapIter = renderMaps
				.iterator();
		while (xml.contains("<repeat>")) {
			...
			String template = xmlToReplace.substring(xmlToReplace
					.indexOf("<template>") + 10, xmlToReplace
					.indexOf("</template>"));
			...
			xml = xml.substring(0, startIndex) + replacement
	...
	private void loadRenderElementsForEachRepeatElement(Node node,
			List<List<Map<String, String>>> renderMaps) throws Exception {
		...
		for (int i = 0; i < list.getLength(); i++) {
			...
			if (n.getNodeName().equalsIgnoreCase("repeat")) {
				...
				if (templateNode == null) {
					throw new IllegalArgumentException(
							"All <repeat> elements must contain a child <template> element.");
				}
	 ...
	 * {@code <macro>, <translations>, and <repeat>)}, by calling the
	 * appropriate tag handler (see {@see
	 * org.openmrs.module.htmlformentry.handler}) for each tag
	 * <p>
	 * 
	 * @param session
	 *            the current form entry session context
	 * @param xml
	 *            the xml string to process
	 * @return the xml string (which should now be html) after tag processing
	 * @throws Exception
	 */
	public String applyTags(FormEntrySession session, String xml)
			throws Exception {
		Document doc = HtmlFormEntryUtil.stringToDocument(xml);
	...
	private void applyTagsHelper(FormEntrySession session, PrintWriter out,
		...
		// Find the handler for this node
		{
			...
			if (name != null) {
				handler = HtmlFormEntryUtil.getService().getHandlerByTagName(
						name);
			}
		}
		...
		// Unless the handler told us to skip them, then iterate over any
		// children
		if (handleContents) {
			if (handler != null && handler instanceof IteratingTagHandler) {
				...
				while (iteratingHandler.shouldRunAgain(session, out, parent,
						node)) {
					NodeList list = node.getChildNodes();
					for (int i = 0; i < list.getLength(); ++i) {
	 ...
	 * <p>
	 * Default behavior is simply to leave the tag unprocessed. That is, any
	 * basic HTML tags are left as is.
	 */
	...
	public void applyNewRepeat(FormEntrySession session, String xml)
		...
		if (context.getMode() == Mode.VIEW || context.getMode() == Mode.EDIT) {
			...
			/*
			 * remove obsgroup /*obsgroup won't display, we only care non group
			 * members Iterator<Obs> it = sortedObs.iterator();
			 * while(it.hasNext()){ if(it.next().hasGroupMembers()){
			 * it.remove(); } }
			 */

			// TODO: now it can't handle when 1 repeat set is a prefix of
			// another
					...
					for (int i = 0; i < rpt.getObsNum(); ++i) {
						if (sortedObs.size() < i + 1
							...
							if (sortedObs.get(i).hasGroupMembers()) {
								...
								// assumption
								// that once the group conceptid match, the
								// group will match
								// skip all the member
								// i+=
								// sortedObs.get(i).getGroupMembers().size();
								int tmp = sortedObs.get(i).getGroupMembers()
										.size();
								for (int j = 0; j < tmp; ++j) {
									sortedObs.remove(i + 1);
								}
								// note the group content in rpt has been
								// skipped in
								// this.FillChildObs
							}
						}
